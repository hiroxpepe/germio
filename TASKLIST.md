...
| Piece | Holds |
| --- | --- |
| `WorldNames` (logic, no Unity at all) | a plain `Dictionary<int, (string Kind, string ID)>`. `Fill(IReadOnlyList<(int InstanceId, string Kind)> found)` builds it whole, once — held true against a repeated `InstanceId` (one `GameObject`, more than one `Collider`), by the indexer (`dict[id] = value`), never `.Add()`, so the last kind matched for that id holds, and nothing throws. **`WorldNames` itself turns each `InstanceId` into its own `ID` string (`g_<InstanceId>`) — the thin edge hands over a plain `int` and a `Kind` alone, never a string made ahead of time.** `NameOf(int instance_id)` reads it — the same true shape as `modio`'s own `INameSource`, `(string, string)`, `("", "")` where the id is not held, never thrown |
| the thin edge (Unity, not built here) | **makes a true, new `WorldNames` at every scene load — never one held over from the scene before, and never a re-`Fill` of an old one.** Walks every collider once; for each, calls `Like()` against the 11 type marks, in a fixed, given order; hands `(GetInstanceID(), matched Kind)` to the new `WorldNames`'s own `Fill` for every collider that truly matched one mark. (`WorldNames.Fill` itself still holds true against a second call, on its own — a real, given safety net, not the true, held way this is used) |
...
**Held wrong at first, set right 2026-09-21 — see `modio`'s own
`docs/sight_checklist.md` §4.8, in that repository, for the full
turn.** This task first held that `germio` itself would turn a
thing toward what it looks at, moving `Sight.eyes` (then thought of
as `modio`'s own name for a `Transform`). That did not hold up, on
two true counts:

+ `Sight` itself moved to `germio` (§4.7), not `modio` — so
  `Sight.eyes`, if it stood at all, would be `germio`'s own name,
  never `modio`'s.
+ **The whole turn is dropped from `germio` outright.** `stemic`
  alone turns a head, through Unity's own Humanoid Look-At IK
  (`OnAnimatorIK`), since that call is only ever made on the
  `GameObject` holding the `Animator` — `Pete`, in `stemic`, never a
  `GameObject` in `germio`. `germio` cannot be the one to call it;
  there is no such call to make from outside.

**`germio`'s own true part, narrowed:** hold `Sight`'s own five
plain values (`reach`, `halfYaw`, `halfPitch`, `eyeHeight`, `eyes`)
as a `MonoBehaviour`, for `modio`'s own sight check to read. `germio`
calls no turn, here or anywhere, for a head. This task is closed —
